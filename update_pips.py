...
def passo_04(p_inst, p_pcs):
    """ Unir as lista [tudo] = [instalados] + [computadores]
...
def passo_07(p_new):
    """ rodar instalação ou upgrade com a lista [desatualizados]
      + [nao_instalados] """
    p_not_approved = []
    p_need_debian = []
    ...
    for pack in p_new:
        ...
        if ret_status > 0:
            ...
            if [list_element_w for list_element_w in lst_results
                    if err_base in list_element_w]:
                print('\tNetwork error during packet processing ' + pack)
                ret_status = 0
            else:
                err_base = 'ERROR: No matching distribution found for'
                if [list_element_e for list_element_e in lst_results
                        if err_base in list_element_e]:
...
print('\tReturned : ' + str(len(pack_not_from_pcs)) +
      ' not pip packs from others PCs\n')

pack_all = passo_04(pack_installed, pack_others_pcs)
pack_new = passo_05(pack_installed, pack_all, pack_outdated)

print('\tExecutar upgrade/instalação de ' + str(len(pack_new)) +
      ' pacotes.' + str(pack_new))
...
if len(pack_not_all) > 0:
    ...
    print('\nPacks inexistente na ou a remover da base PIP ou pack Debian ' +
          MSG_PART_I)
