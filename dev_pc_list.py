...
def store_dev_packs(connection, machine, packs):
    ...
    for pack in packs:
        sql = 'insert or replace into main.debian_packs ' + \
            '(machine, dev_pack_name)' + \
            ' values (' + "'" + machine + "','" + pack + "');"
...
def create_install_dev(file_name, dev_all):
    ...
    with open(file_name, 'w', encoding="ascii") as file_obj:
        ...
        # first line needs a little push ;)
        file_obj.write('sudo apt install --assume-yes ')
...
SQL_DIST = SQL_DIST + \
    ' select dev_pack_name FROM debian_packs where machine = '
